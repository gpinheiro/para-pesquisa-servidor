...
describe ImportController do
  ...
  context 'successful requests' do
    ...
    it 'should import submissions without subtitution and identifier' do
      job_id = parse_sample
      post :import, default_params.merge(job_id: job_id, form_id: form.id, grouping: 'Atribuição de Jovem')
      assert_response :success

      expect(json_response['successful_imports']['1'][0]['id']).to eq(1)
      expect(json_response['successful_imports']['2'][0]['id']).to eq(2)
      expect(json_response['successful_imports']['3'][0]['id']).to eq(3)
    end
    ...
    it 'should import submissions with substitutions' do
      ...
      expect(json_response['successful_imports']['1'][0]['id']).to eq(1)
      expect(json_response['successful_imports']['1'][1]['id']).to eq(3)
      expect(json_response['successful_imports']['2'][0]['id']).to eq(2)
    end
